...
const toggleResultVisibility = (orderId?: number) => {
  if (!orderId) return;
  const next = new Set(expandedResultOrders.value);
  if (next.has(orderId)) {
    next.delete(orderId);
  } else {
    next.add(orderId);
  }
  expandedResultOrders.value = next;
};
...
const printServiceOrder = async (order: ServiceOrder) => {
  if (!order.id) return;
  try {
    const blob = await fetchServiceOrderPdf(order.id);
    const url = URL.createObjectURL(blob);
    const printWindow = window.open(url);
    if (!printWindow) {
      showToast('error', 'Trình duyệt chặn cửa sổ in. Vui lòng cho phép pop-up.');
    ...
    const errorMessage = err?.response?.data?.message ?? err?.message ?? 'Không thể in phiếu CLS.';
    showToast('error', errorMessage);
  }
};
              ...
              <div class="rounded-xl border border-emerald-100 bg-emerald-50/40 p-4">
                <div class="flex items-center justify-between">
                  ...
                  <button
                    ...
                    class="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wide text-slate-700 shadow-sm transition hover:bg-slate-50"
                  >
                    <svg
