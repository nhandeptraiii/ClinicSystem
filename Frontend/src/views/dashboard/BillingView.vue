...
const canModifyBilling = computed(() => authStore.hasRole(['ADMIN', 'RECEPTIONIST']));
...
const handledeleteBilling = async () => {
  if (!selectedBilling.value) return;
  const confirmDelete = window.confirm('Bạn có chắc chắn muốn xóa hóa đơn này không? Hành động này không thể hoàn tác.');
  if (!confirmDelete) return;

  try {
    await deleteBilling(selectedBilling.value.id);
             ...
             <select
              ...
              >
                <option v-for="filter in statusFilterPills" :key="filter.value" :value="filter.value">
                   {{ filter.label }} ({{ statusCounts[filter.dataKey as keyof typeof statusCounts] }})
                </option>
              </select>
          </div>
        </div>
        ...
        <div class="grid gap-6 xl:grid-cols-[1.5fr_1.2fr]">
          ...
          <div class="rounded-3xl border border-emerald-100 bg-white/90 p-6 shadow">
            ...
            <div v-else>
              ...
              <div v-if="isAdmin" class="mt-8 border-t border-slate-200 pt-6">
                 <button
                    type="button"
                    class="w-full inline-flex justify-center items-center gap-2 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-600 shadow-sm transition hover:bg-rose-100/80 hover:border-rose-300"
                    @click="handledeleteBilling"
                  >
                  ...
                  <p class="mt-2 text-center text-xs text-slate-500">
                    Hành động này sẽ xóa hoàn toàn hóa đơn khỏi hệ thống và không thể hoàn tác.
                  </p>
              </div>
              </div>
            </div>
